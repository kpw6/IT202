        ...
        if ($result) {
            ...
            if ($balance >= $fee) {
                ...
                if ($r) {
                    ...
                    if($r) {
                        flash("PointsHistory filled successfully");
                    }
                    else {
                         flash("Problem filling pointshistory table");
                    }
                    ...
                    $stmt = $db->prepare("UPDATE Competitions SET reward = SUM(fee) where id = :id && expires > current_timestamp && paid_out = 0");
                    $r = $stmt->execute([":id" => $_POST["cid"]]);
                    if($r) {
                      flash("Reward Updated");
                    }
                    else {
                      flash("Error updating reward");
                    }
                    die(header("Location: #"));
                }
                else {
